...
void RoomsPage::openRoomDialog()
{
    ...
    if (maxMembers < 2 || maxMembers > 100) {
        NotificationDialog dialog("Количество участников должно быть от 2 до 100", NotificationDialog::Error, this);
        dialog.exec();
        return;
    }
    ...
    if (roomId != -1) {
        NotificationDialog dialog("Комната создана!", NotificationDialog::Success, this);
        dialog.exec();
        loadRooms();
    } else {
        NotificationDialog dialog("Не удалось создать комнату", NotificationDialog::Error, this);
        dialog.exec();
    }
}
...
void RoomsPage::onJoinRoom(int roomId)
{
    if (m_roomManager->joinRoom(roomId)) {
        NotificationDialog dialog("Вы присоединились к комнате!", NotificationDialog::Success, this);
        dialog.exec();
        loadRooms();
    } else {
        NotificationDialog dialog("Не удалось присоединиться", NotificationDialog::Error, this);
        dialog.exec();
    }
}
